...
## Import ABIs
    ...
    import BoardABI from "@luckymachines/game-core/games/hexploration/abi/HexplorationBoard.json";
    import QueueABI from "@luckymachines/game-core/games/hexploration/abi/HexplorationQueue.json";
    import GameplayABI from "@luckymachines/game-core/games/hexploration/abi/HexplorationGameplay.json";
    import GameSetupABI from "@luckymachines/game-core/games/hexploration/abi/GameSetup.json";
    import CardDeckABI from "@luckymachines/game-core/games/hexploration/abi/CardDeck.json";
    import GameTokenABI from "@luckymachines/game-core/games/hexploration/abi/GameToken.json";
    import PlayZoneSummaryABI from "@luckymachines/game-core/games/hexploration/abi/PlayZoneSummary.json";
...
```
                          ┌──────────────────────┐
                          │  HexplorationController│  (Player entry point)
                          └──────────┬───────────┘
                                     │
                    ┌────────────────┼────────────────┐
                    ▼                ▼                ▼
           ┌──────────────┐  ┌─────────────┐  ┌──────────────┐
           │HexplorationBoard│  │HexplorationQueue│  │  GameSetup   │
           └──────┬───────┘  └──────┬──────┘  └──────┬───────┘
                  │                 │                 │
        ┌─────┬──┴──┬─────┐       │          Chainlink VRF
        ▼     ▼     ▼     ▼       ▼
   ┌───────┐┌───────┐┌───────┐ ┌──────────────────┐
   │CardDeck││GameToken││PlayZone│ │HexplorationGameplay│
   └───────┘└───────┘└───────┘ └──────────────────┘
                                        │
                                        ▼
                                 ┌─────────────┐
...
| **Contract** | **Role** | **ABI** |
| --- | --- | --- |
| HexplorationController | Player-facing entry point for creating, joining, and playing games | Yes |
| HexplorationBoard | Core game board state: zones, players, relics, game lifecycle | Yes |
| HexplorationQueue | Turn queue management, action submission, VRF randomness | Yes |
| HexplorationGameplay | Action resolution engine, play-through processing, state updates | Yes |
...
| **Contract** | **Address** |
| --- | --- |
| HEXPLORATION_BOARD | `0x52771fad873a64Ed6c5385f74d81F99f2106fb19` |
| HEXPLORATION_CONTROLLER | `0x5515C1158a84CAb258d2d8c294c4b7C69AEf0BBc` |
...
All deployment addresses across networks can be found in [games/hexploration/deployments.json](https://github.com/LuckyMachines/game-core/blob/docs/games/hexploration/deployments.json).
...
1. **Create**: Anyone calls `HexplorationController.requestNewGame()` with a player limit (1-4).
2. **Register**: Players call `HexplorationController.registerForGame()` to join an open game.
...
4. **Play**: Players submit actions each turn via `HexplorationController.submitAction()`. Actions are queued, randomness is requested, and outcomes are resolved.
5. **End**: The game ends when all players escape, are defeated, or the end-game scenario completes. `HexplorationBoard.setGameOver()` is called.

## Turn Cycle

Each turn follows a processing pipeline managed by the `HexplorationQueue`:
...
3. **PlayThrough**: After randomness is fulfilled, `HexplorationGameplay.processPlayThrough()` resolves all player actions using the random values.
...
Hexploration uses **Chainlink VRF** (Verifiable Random Function) for provably fair randomness:

1. `HexplorationQueue.requestProcessActions()` → requests random words from Chainlink VRF
...
- [Hexploration Board](#hexploration-board-hexplorationboardsol): Core game board state management, zones, players, relics.
- [Hexploration Queue](#hexploration-queue-hexplorationqueuesol): Turn queue management, action queuing, VRF randomness.
- [Hexploration Gameplay](#hexploration-gameplay-hexplorationgameplaysol): Action resolution engine and play-through processing.
...
See the [Architecture Overview](#deployed-contracts-sepolia) for a full table of Sepolia addresses, or [games/hexploration/deployments.json](https://github.com/LuckyMachines/game-core/blob/docs/games/hexploration/deployments.json) for all networks.
...
## Hexploration Board (HexplorationBoard.sol)
...
## Hexploration Queue (HexplorationQueue.sol)
...
## Hexploration Gameplay (HexplorationGameplay.sol)
...
`(address)boardAddress`: Address of the HexplorationBoard contract.
...
Both `HexplorationGameplay` and `GameSetup` implement the `shouldProgressLoop` / `progressLoop` pattern, making them compatible with **Chainlink Automation** (formerly Chainlink Keepers).
...
1. **Players submit actions** → actions queued in `HexplorationQueue`
2. **All players submitted (or time limit)** → `requestProcessActions()` triggers VRF request
3. **VRF fulfilled** → `shouldProgressLoop()` returns `true` on `HexplorationGameplay`
...
The `HexplorationQueue` tracks idle turns per player via `idleTurns(queueID, playerID)`. If a player does not submit an action for **3 consecutive turns**, they are kicked from the game:

- `HexplorationBoard.setPlayerInactive()` is called
...
Used by `HexplorationGameplay.processPlayerActions()` to describe the batch sizes of state updates to apply.
